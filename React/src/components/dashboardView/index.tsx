...
function DashView() {
  ...
  const [stats, setStats] = useState<Stats>({ total_jobs: 0, total_users: 0, total_applications: 0 })
  const [selectedData, setSelectedData] = useState<(UserData | JobData | ApplicationData)[]>([])
  const [loading, setLoading] = useState(false)
  const [showModal, setShowModal] = useState(false)
  const [modalTitle, setModalTitle] = useState("")
  const [searchQuery, setSearchQuery] = useState("")
  const [searchType, setSearchType] = useState<"job" | "user" | "none">("job")
  const [filteredData, setFilteredData] = useState<(UserData | JobData | ApplicationData)[]>([])
  const [platformActivityData, setPlatformActivityData] = useState<ChartDataPoint[]>([])
  ...
  const fetchPlatformActivity = useCallback(async () => {
    try {
      ...
      const response = await axios.get<ChartDataPoint[]>("http://localhost:8000/stats/daily", {
        headers: { Authorization: `Bearer ${token}` },
        timeout: 10000,
      })
  ...
  const fetchJobCategories = useCallback(async () => {
    try {
      ...
      const response = await axios.get<ApiJobCategoryResponse[]>("http://localhost:8000/admin/stats/job-categories", {
        headers: { Authorization: `Bearer ${token}` },
        timeout: 10000,
      })
      ...
      const formatted: JobCategory[] = response.data.map((item: ApiJobCategoryResponse, index: number) => ({
  ...
  const handleSearch = useCallback(() => {
    ...
    const filtered = selectedData.filter((item) => {
      if (searchType === "user") {
        ...
        return userData.username?.toLowerCase().includes(query) || userData.email?.toLowerCase().includes(query)
  ...
  const handleCardClick = useCallback(async (endpoint: string, title: string) => {
    const token = localStorage.getItem("access_token")
    if (!token) {
  ...
  const handleDeleteUser = useCallback(
    async (userId: string) => {
      ...
      try {
        ...
        const updatedData = filteredData.filter((user) => (user as UserData).id !== userId)
        setFilteredData(updatedData)
        setSelectedData(selectedData.filter((user) => (user as UserData).id !== userId))
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans" onKeyDown={handleKeyDown}>
      {error && (
              ...
              <div className="bg-white rounded-xl shadow-lg w-11/12 md:w-3/4 lg:w-1/2 max-h-[80vh] overflow-y-auto p-6 relative">
                ...
                {searchType !== "none" && (
                  <div className="flex gap-2 mb-6">
                    <input
                      type="text"
                      placeholder={searchType === "user" ? "Search user by username..." : "Search job by title..."}
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      onKeyDown={(e) => e.key === "Enter" && handleSearch()}
                      className="flex-1 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                      aria-label={searchType === "user" ? "Search users" : "Search jobs"}
                    />
                    <Button
                ...
                {/* Results */}
                {loading ? (
                  <div className="text-center py-10 text-gray-500 animate-pulse" role="status" aria-live="polite">
                    Loading...
                  </div>
                          ...
                          <tr key={idx} className="hover:bg-gray-50">
                            {modalTitle === "Total Application" ? (
                              ...
                              Object.entries(item).map(([key, value], i) => (
                                <td key={i} className="border border-gray-300 px-4 py-2 text-gray-900">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    setDeleteConfirm({
                                      ...
                                      username:
                                        (item as UserData).username || (item as UserData).email || "Unknown User",
                                    })
                                  }
                                  className="flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-red-500"
                                  aria-label={`Delete user ${(item as UserData).username || (item as UserData).email}`}
                                >
          ...
          {deleteConfirm.show && (
            ...
            >
              <div className="bg-white rounded-xl shadow-lg w-96 p-6">
                <h3 id="delete-title" className="text-lg font-bold text-gray-900 mb-4">
                  Confirm Delete
                </h3>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={platformActivityData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <Tooltip
                          ...
                          formatter={(value, name) => [value, name === "jobPosting" ? "Job Postings" : "Applications"]}
                        ...
                        <Line
                          ...
                          activeDot={{ r: 6, stroke: "#ffc107b8", strokeWidth: 2 }}
                        />
                      </LineChart>
                    </ResponsiveContainer>
                  ) : (
              ...
              <CardContent className="p-4 lg:p-6 pt-0">
                ...
                <div className="mt-4 space-y-2 max-h-32 overflow-y-auto">
                  {jobCategoriesData.map((item, index) => (
                    <div key={index} className="flex items-center justify-between text-xs lg:text-sm py-1">
