...
function Employer() {
  ...
  .map((job, idx) => (
    <motion.div
      ...
      whileInView={{ opacity: 1, y: 0 }} // Animate to visible, move up
      viewport={{ once: true, amount: 0.2 }} // Trigger when 20% of card visible
      transition={{ duration: 0.5, delay: idx * 0.1 }} // stagger cards
    >
      ...
      <div className="flex flex-col gap-4 ml-4">
        <button onClick={() => handleOpenForm(job)} className="bg-primary text-white px-4 py-1 rounded-md hover:bg-green-600 transition-all duration-300">
          Update
        </button>
        <button onClick={() => handleDelete(job.id)} className="bg-red-500 text-white px-4 py-1 rounded-md hover:bg-red-600 transition-all duration-300">
          Delete
        </button>
      </div>
     </motion.div>
