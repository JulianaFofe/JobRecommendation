  ...
  return (
    <section className="flex-1 bg-gray-100">
      <div className=" mx-auto p-6 px-20">
        ...
        {/* Filters */}
        <div className="flex-1 flex flex-wrap gap-26 space-x-20 mb-15">
          {["Industry", "Salary Range", "Location"].map((label) => (
            <button
              key={label}
              className="flex justify-betwee items-center cursor-pointer gap-2 bg-white border border-white rounded-md px-30 py-2 shadow-sm"
            >
        ...
        <div className="space-y-4">
          {cards.map((c) => (
            <article key={c.id} className="bg-white rounded-md shadow px-4 py-4 flex  items-center justify-between">
              <div className="pr-3">
